...
void WaveformPainter::drawWaveform(Painter& p,
  ...
  const BlockSet& selection = waveform_->model().selection();
  ...
  while (getSize(r) > 0) {
    ...
    int x1 = waveform_->model().timeToX(Samples<44100>(draw.first));
    int x2 = waveform_->model().timeToX(Samples<44100>(draw.second));
