...
void WaveformPainter::drawWaveform(Painter& p,
  ...
  while (getSize(r) > 0) {
    ...
    int x1 = waveform_->timeToX(Samples<44100>(draw.first));
    int x2 = waveform_->timeToX(Samples<44100>(draw.second));
...
void WaveformPainter::drawGrid(Graphics& g, const Range<Samples<44100> >& r) {
  ...
  for (int i = b - 1; i <= e + 1; ++i) {
    ...
    String s = formatTime(time, waveform_->length_, false, false, decimals);
