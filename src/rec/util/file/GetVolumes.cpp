...
void addAudioCDs(VirtualFileList* volumes) {
    auto names = AudioCDReader::getAvailableCDNames();
    for (int i = 0; i < names.size(); ++i) {
        std::unique_ptr<AudioCDReader> reader(
            AudioCDReader::createReaderForCD(i));
        if (reader && reader->isCDStillPresent() && reader->getNumTracks()) {
            auto key = str(cd::getCDKey(reader.get()));
            if (key.size())
