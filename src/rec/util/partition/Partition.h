...
typedef Partition<const juce::Array<File>, juce::Array<int>, String>
  JucePartition;

typedef Partition<const std::vector<string>, std::vector<int>, string>
  STLPartition;

template <>
inline String JucePartition::getName(int i) const {
  return kids_[i].getFileName();
}

template <>
inline string STLPartition::getName(int i) const {
  return kids_[i];
}

template <>
inline void JucePartition::add(int i) { list_->add(i); }

template <>
inline void STLPartition::add(int i) { list_->push_back(i); }
