#include "rec/gui/Table.h"
#include "rec/widget/status/Time.h"
#include "rec/data/proto/Proto.h"

using rec::widget::status::time::formatTime;

namespace rec {
namespace gui {

TableModelBase::TableModelBase(const Address& a, const TableColumnList& c)
    : address_(a), columns_(c) {}
...
int TableModelBase::getNumRows() {
  ScopedLock l(lock_);
  return proto::getSize(address_, message());
}
...
String TableModelBase::displayText(const TableColumn& col, const Value value) {
  switch (col.type()) {
   ...
   case TableColumn::TIME: return formatTime(value, false, false);
