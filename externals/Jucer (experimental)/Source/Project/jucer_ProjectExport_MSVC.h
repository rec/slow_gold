  ...
  const String getPreprocessorDefs (const Project::BuildConfiguration& config, const String& joinString) const
    {
        ...
        removeUnderscoresFromPreprocessorDefs(&defines);
        ...
        for (int i = 0; i < defines.size(); ++i)
        {
            ...
            std::cout << def.toCString() << std::endl;
            const String value (defines.getAllValues()[i]);
            if (value.isNotEmpty())
                def << "=" << value;
            std::cout << "def! " << def.toCString() << "\n";
            result.add (def);
        }
...
protected:
    ...
    void createConfig (XmlElement& xml, const Project::BuildConfiguration& config) const
    {
...

        {
            ...
            compiler->setAttribute ("AdditionalIncludeDirectories",
                                    replacePreprocessorTokens (config, getHeaderSearchPaths (config).joinIntoString (";")));
            String s = getPreprocessorDefs (config, ";");
            std::cout << "s! " << s.toCString() << "\n";
            compiler->setAttribute ("PreprocessorDefinitions", s);
...
protected:
    ...
    void fillInProjectXml (XmlElement& projectXml)
    {
        ...
        for (int i = 0; i < project.getNumConfigurations(); ++i)
        {
            ...
            cl->createNewChildElement ("AdditionalIncludeDirectories")->addTextElement (getPreprocessorDefs (config, includePaths.joinIntoString (";")));
